...
class AtomGraph(object):
  def __init__(self, adj_list, colors, coeffs, kind0, kind1):
    """
    ...
    coeffs (dict) : A dictionary of the various bond coefficients we have precalculated, using coeffs.py
  ...
  def getAllCNs(self):
    ...
    """
    return np.array([entry.size for entry_list in self.adj_list])
  ...
  def getHalfBond(self, atom_key, bond_key):
    ...
    atom2 = self.colors[self.adj_list[key]]
