...
def randomizer(atoms):
    ...
    t = time.clock()
    meancelist1, stdlist1, totalloops, mincelist1, maxcelist1, minatomlist1, maxatomlist1 = ce_shuffle.atomchanger(
        atoms, 'Cu', 'Ag')
    ...
    t2 = time.clock()
    print('1= ' + str(t2 - t))
    meancelist2, stdlist2, totaloops, mincelist2, maxcelist2, minatomlist2, maxatomlist2 = ce_shuffle.atomchanger(atoms,
                                                                                                                  'Au',
                                                                                                                  'Cu')
    t2 = time.clock()
    print('2= ' + str(t2 - t))
    meancelist3, stdlist3, totalloops, mincelist3, maxcelist3, minatomlist3, maxatomlist3 = ce_shuffle.atomchanger(
        atoms, 'Ag', 'Au')
    t2 = time.clock()
...
def boundce(minatomlist, maxatomlist, shellnums, morphologyname):
    ...
    for i in range(0, 63 * len(shellnums)):
        ...
        namemin = morphologyname + 'Shell_#' + str(k) + str(el1list[i]) + str(perclist1[i]) + str(el2list[i]) + str(
            100 - perclist1[i]) + 'min.xyz'
        namemax = morphologyname + 'Shell_#' + str(k) + str(el1list[i]) + str(perclist1[i]) + str(el2list[i]) + str(
            100 - perclist1[i]) + 'max.xyz'
